...
class MFeat_Dataset(Dataset):
    def __init__(self, root:str, feats:list=None, partials:list=None, download:bool=False):
        """
        ...
        :param download: If true, downloads the dataset from the internet and
            ...
            downloaded again.
        """
        self.resources = ["https://archive.ics.uci.edu/ml/machine-learning-databases/mfeat/mfeat.tar"]
        self.root = root
        if download:
            self.download()
    ...
    def __getitem__(self, index):
        views = [
            self.dataset[feat][index]
            for feat in self.feats
        ]
        partials = [
            self.dataset[partial][index]
